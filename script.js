...
const bucket = Math.floor(new Date(s.created_at).getTime() / 60000); 
...
case 'top3':        return false; 
case 'top1':        return false; 
case 'league_size': return false; 
case 'comeback':    return false; 
...
if(Math.abs(from-val)<0.05) return; 
...
P.sessions.forEach(s=>{
const t=sessTs(s);
if(!t||isNaN(t)||t<weekStart) return; 
const dayOfWeek=Math.floor((t-weekStart)/86400000); 
...
const makeRing=(pct,color,size=56)=>""; 
const kpis=[
...
const wCut=new Date(getWeekStart()).toISOString(); 
...
const today = new Date().toDateString();
if(today === _lastRenderedDateStr) return; 
...
'linear-gradient(135deg,#200010,#100020,#200010)',
'linear-gradient(135deg,#1A0A00,#200010,#001020)', 
'linear-gradient(135deg,#0A0A1A,#1A0A1A,#0A1A1A)', 
];
...
if(!f.notes) f.notes=[];
f.notes=f.notes.filter(n=>n&&n.id); 
});
});
}
function _kbSave3() {
...
const m = document.getElementById('k3meta-wc'); 
...
avatarFailed: a.avatarFailed || false   
}))};
...
if(!avatarUrl) found.avatarFailed = true; 
}
} catch(e) {
const found = _auth.authors.find(a => a.id === newAuthor.id);
...
const selectors = '.mg-item,.cat-item,.insight-card,.rec-card,.rec-item,.csg-item'; 
